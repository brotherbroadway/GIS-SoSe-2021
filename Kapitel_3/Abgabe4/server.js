...
(function (P_3_4Server) {
    ...
    async function handleRequest(_request, _response) {
        ...
        if (_request.url) {
            ...
            if (chosenPath == "/send") { // path used when button is pressed to send data to the database
                ...
                let myData = await checkDB(databaseUrl); // waits to receive entry from database
                console.log(myData);
                let stringJSON = JSON.stringify(myData);
    ...
    async function saveMe(_url, _formEntry) {
        let options = { useNewUrlParser: true, useUnifiedTopology: true };
        let dbClient = new Mongo.MongoClient(_url, options);
